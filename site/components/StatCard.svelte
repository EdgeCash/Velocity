...
<style>
  ...
  .value.pos { color: var(--v-pos, #3fb950); }
  .value.neg { color: var(--v-neg, #f0616a); }
  .sub {
    display: flex;
  ...
  .sub-text { color: var(--v-ink-3, #64748b); }
  .delta {
    font-weight: 600;
    font-variant-numeric: tabular-nums;
    white-space: nowrap;
  }
  .delta.pos { color: var(--v-pos, #3fb950); }
  .delta.neg { color: var(--v-neg, #f0616a); }
  .delta.flat { color: var(--v-ink-3, #64748b); }
  /* The arrow is the point: it says up or down without relying on hue. */
  .arrow { font-size: 0.62em; }
  .meter {
    height: 3px;
    border-radius: 999px;
    background: var(--v-line, #1c2733);
    overflow: hidden;
  ...
  .meter.warm .fill { background: var(--v-warn, #d29922); }
  .meter.hot .fill { background: var(--v-neg, #f0616a); }
