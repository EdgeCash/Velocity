...
scores come from the league's schedule feed (nflverse/CFBD), keyed by its own
game id. The two id spaces don't line up, so grading needs a bridge: match each
slate game to a final by the resolved home/away **team codes** and the **game
date** (both feeds carry team names and a kickoff). That's the same tolerant
team resolution the slate itself uses, so a game that resolves for pricing
resolves for grading.
...
def finals_for_slate(
    ...
    *,
    ...
    """Map schedule-feed finals onto the slate's ``game_id`` by team code + date.

    ``games_map`` has ``game_id``, ``home_team``, ``away_team``, ``kickoff`` (the
    slate side); ``schedule`` is a ``Games``-shaped frame (schedule-feed side)
    with team names, ``kickoff``, and ``home_score``/``away_score``. Returns
    ``[game_id, home_score, away_score]`` for every slate game matched to a
    played game.
    """
