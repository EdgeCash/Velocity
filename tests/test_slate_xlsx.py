...
def test_projections_display_builds_expected_columns() -> None:
    ...
    row = df.iloc[0]
    assert row["Matchup"] == "Buffalo Bills @ Kansas City Chiefs"
...
def test_plays_display_joins_teams_and_derives_stake() -> None:
    ...
    assert disp.iloc[0]["Matchup"] == "Buffalo Bills @ Kansas City Chiefs"
...
def test_export_workbook_writes_readable_sheets(tmp_path: Path) -> None:
    ...
    out = export_slate_workbook(
        ...
        None,
        league="nfl", generated_at="2026-09-10 00:00:00", bankroll=100.0,
    )
    ...
    assert "Kansas City Chiefs" in plays["Matchup"].to_string()
    assert (plays["Model %"].dropna() > 0).all()


def test_export_includes_prop_sheet_when_present(tmp_path: Path) -> None:
    props = pd.DataFrame({
        "game_id": ["g1"], "player": ["Josh Allen"], "market": ["pass_yards"],
        "side": ["over"], "point": [249.5], "book": ["dk"], "price": [-115],
    ...
    assert "Josh Allen" in sheets["Prop Suggestions"]["Player"].to_string()
